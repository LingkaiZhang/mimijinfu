...
import com.nongjinsuo.mimijinfu.adapter.NewsAndTrendsAdapter;
...
public class NewsAndTrendsActivity extends AbstractActivity implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private NewsAndTrendsAdapter newsAndTrendsAdapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_news_and_trends);
        ButterKnife.bind(this);
        titleView.setText("新闻动态");
        init();
        addListener();
    }
    ...
    private void loadDate(boolean isRefresh, boolean isFirst) {
        ...
        JacksonRequest<NewsModel> jacksonRequest = new JacksonRequest<>(RequestMap.findList(pageNum,"news"), NewsModel.class, new Response.Listener<NewsModel>() {
            @Override
                        ...
                        if (baseVo.getResult().getData().size() == 0) {
                            ...
                            if (pageNum == 0) {
                                if (newsAndTrendsAdapter ==null){
                                    newsAndTrendsAdapter = new NewsAndTrendsAdapter(NewsAndTrendsActivity.this,activityBeanList);
                                    listView.setAdapter(newsAndTrendsAdapter);
                                }else{
                                    newsAndTrendsAdapter.setDataBeen(activityBeanList);
                                }
                                if (baseVo.getResult().getData().size() < 20) {
                                    swipeRefreshLayout.setIsShowLoadingMoreView(false);
                                }
                            } else {
                                newsAndTrendsAdapter.setMoreDateBeen(activityBeanList);
                            }
                        }
                        //判断当前账户是否在其它设备登录
//                        Util.loginPrompt(getActivity(), baseVo.result.loginPrompt);
                        ...
                        if (pageNum == 0) {
                            ...
                            rlNoDataView.setVisibility(View.VISIBLE);
                            ivNoData.setImageResource(R.drawable.img_wnr_49);
                            tvNoData.setText(NewsAndTrendsActivity.this.getString(R.string.str_no_data));
                            tvRefresh.setVisibility(View.GONE);
                        } else {
                            Toast.makeText(NewsAndTrendsActivity.this, "加载完成", Toast.LENGTH_SHORT).show();
                            swipeRefreshLayout.setIsShowLoadingMoreView(false);
                        }
                    }
                }
    ...
    private void netWorkFail(int flag) {
        ...
        if (flag == 0) {
            ...
            Toast.makeText(NewsAndTrendsActivity.this, getString(R.string.str_no_network), Toast.LENGTH_SHORT).show();
        } else {
