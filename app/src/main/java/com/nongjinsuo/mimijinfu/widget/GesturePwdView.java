...
public class GesturePwdView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //TODO 调整手势点位之间的间距
        super.onMeasure(widthMeasureSpec- PxAndDip.dip2px(getContext(),80), heightMeasureSpec-PxAndDip.dip2px(getContext(),80));
    }
        ...
        if (canContinue) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    if (linedCycles.size() < minDianNum && validationStatus != GesturePwdStatus.VALIDATE_AGAIN) {// 最少连接四个点,并且不处于二次验证状态
                        result = false;
...
//                    drawOutsideCycle(cycles[i], canvas, NORMAL_COLOR);
                    drawInnerCycle(cycles[i], canvas, NORMAL_COLOR);
                }
            } else
            // 绘画中
            {
...
//        paintNormal.setColor(color);
//        canvas.drawCircle(lockCircle.getOx(), lockCircle.getOy(), lockCircle.getR() / 3f, paintNormal);
    }
