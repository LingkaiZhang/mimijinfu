...
public class PinnedHeaderExpandableListView extends ExpandableListView implements OnScrollListener,OnGroupClickListener {
	...
	private P2pAndProject2Adapter mAdapter;
	...
	public void setHeaderData(int groupPosition)
	{
		if(groupPosition<0)
			return ;
		DingTouHuoTouModel.ResultBean.ListBean listBean =  mAdapter.getGroup(groupPosition);
		ImageView ivIcon = (ImageView) mHeaderView.findViewById(R.id.ivIcon);
		TextView tvTitle = (TextView) mHeaderView.findViewById(R.id.tvTitle);
		TextView tvMore = (TextView) mHeaderView.findViewById(R.id.tvMore);
...
//		Toast.makeText(getContext(),"分组："+groupPosition,Toast.LENGTH_SHORT).show();
		if (groupPosition == 0){
			Intent intent = new Intent(getContext(), DingTouBaoListActivity.class);
			getContext().startActivity(intent);
		}
		else{
			Intent intent = new Intent(getContext(), HuoTouBaoListActivity.class);
			getContext().startActivity(intent);
...
//		this.setSelectedGroup(groupPosition);
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		...
		mAdapter = (P2pAndProject2Adapter) adapter;
	}
